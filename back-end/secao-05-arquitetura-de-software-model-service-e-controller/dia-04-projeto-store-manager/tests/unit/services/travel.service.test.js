...
describe('Realizando testes - TRAVEL SERVICE:', function () {
  ...
  it('Alterando travel(status da corrida) com sucesso', async function () {
    ...
    const inputTravelId = 42;
    const inputDriverId = 1;
    const responseService = await travelService.updateTravelStatus(inputDriverId, inputTravelId);

    expect(responseService.status).to.equal('SUCCESSFUL');
    expect(responseService.data).to.be.deep.equal(responseData);
  });

  it('Não altera travel quando passado um driverId inexistente', async function () {
    sinon.stub(driverModel, 'findById').resolves(undefined);

    const inputTravelId = 42;
    const inputDriverId = 9999999;
